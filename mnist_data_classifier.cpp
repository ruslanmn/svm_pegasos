...
void convert_to_cl_float(uint8_t* src, cl_float* dst, size_t size) {
...
MnistDataClassifier::MnistDataClassifier(const char* train_images_filename,
    ...
    for(size_t i = 0; i < 10; i++) {
        train_images_[i] = (cl_float*) malloc(sizeof(cl_float) * weight_size * number_sizes[i]);
    }
    ...
    for(size_t i = 0; i < data_size; i++) {
        ...
        convert_to_cl_float(&images[i * weight_size], &train_images_[number][image_index * weight_size], weight_size);
...
void MnistDataClassifier::load_svm_classes(cl_float h, size_t batch_size, cl_context context, cl_device_id device_id) {
    int n = 10;
    ...
    for(int i = 0; i < n; i++) {
        ...
        for(int j = 0; j < n ; j++) {
            if (j <= i)
                ...
                cl_float* x = (cl_float*) malloc( size * weight_size * sizeof(cl_float) );
                memcpy(x, train_images_[i], number_sizes[i] * weight_size * sizeof(cl_float));
                memcpy(&x[number_sizes[i] * weight_size], train_images_[j], number_sizes[j] * weight_size * sizeof(cl_float));
...
uint8_t MnistDataClassifier::predict(uint8_t* x_uint) {
    size_t n = 10;
    size_t weight_size = mdl.get_weight_size();
    cl_float x_cl_float[weight_size];
    convert_to_cl_float(x_uint, x_cl_float, weight_size);
    ...
    for(size_t i = 0; i < n; i++)
        for(size_t j = i + 1; j < n; j++) {
            res = svm_classes[i][j]->predict(x_cl_float);
            if( res >= 1 )
                counts[i]++;
            else if (res <= -1)
                counts[j]++;
...
int main() {
    ...
    cl_float* x[mdc.number_sizes[0] + mdc.number_sizes[1]];
    memcpy(x, mdc.train_images_[0], mdc.number_sizes[0] * sizeof(cl_float*));
    memcpy(&x[mdc.number_sizes[0]], mdc.train_images_[1], mdc.number_sizes[1] * sizeof(cl_float*));

    cl_float y[mdc.number_sizes[0] + mdc.number_sizes[1]];
    ...
    cl_float** test_x = (cl_float**)malloc(sizeof(cl_float*) * test_data_size);
    for(int i = 0; i <  test_data_size; i++) {
        test_x[i] = (cl_float *) malloc(sizeof(cl_float) * mdl.get_weight_size());
        for(int p = 0; p < mdl.get_weight_size(); p++)
            test_x[i][p] = (cl_float)mdl.get_test_images()[i][p];
    }
    ...
    for(int i = 0; i < test_data_size; i++) {
        ...
        cl_float c = svm.predict(test_x[i]);
